 ...
 * @param {props} props
 */
function GameComponent_old(props) {
    ...
    const extractInterests = async() => {

        let interestsBackend = await UserService.getInterests();
        console.log(interestsBackend);
        setAllInterests(interestsBackend[0].facebookInterests);
        // await UserService.getInterests().then(function(interestsBackend) {
        //     console.log(interestsBackend);
    ...
    const extractWheelInterests = async() => {
        //console.warn("COMMON INTERESTS")
        //console.warn(commonInterests);

        let ints = await UserService.getInterests()
...
export default GameComponent_old;